...
useEffect(() => {
  ...
  const fetchTasksFromBackend = async () => {
    try {
      // 1. Attempt to fetch tasks for the specific logged-in user
      const response = await axios.get(`http://localhost:5001/api/tasks?username=${computedUser}`);
      
      if (response.data && response.data.length > 0) {
        setTasks(response.data);
      } else {
        // 2. FALLBACK: If empty, fetch everything so your dashboard never renders completely blank during local testing
  ...
  // Debug tool to audit the structure of tasks safely in the console
  useEffect(() => {
    if (tasks.length > 0) {
  ...
  // Safely checks if a task is completed based on backend format variants
  const isTaskFinished = (task) => {
    if (task.status === 'completed') return true;
    
    // Normalize subtasks from backend naming convention variants
    const subtaskList = task.subtasks || task.tactical_subtasks || [];
    if (subtaskList.length > 0) {
      // If it's objects like { text, done }, test .done. Otherwise, if it's plain strings, wait for manual clear button
  ...
  const toggleSubtask = async (taskId, subtaskIdx, subtaskText) => {
    ...
    // Convert subtasks array uniformly into objects to preserve state changes
  ...
  // Automated Formulaic Clutch Score Engine (Deterministic Score 0-100)
  const calculateClutchScore = (task) => {
    if (clutchOverrides[task.id || task._id]) return 92; // Max emergency trigger lock
    let score = 30;
  ...
  // Dynamic Level Progression Calculation Engine
 ...
 // 🎛️ CORE THEME UTILITY STYLE RESOLVER
  ...
  let themeCardClass = "bg-[#0c1220] text-slate-100"; // Remapped out raw border-slate-800
  let themeHeaderClass = "bg-[#0c1220]/80"; // Remapped out raw border lines
  ...
  // Find the single overall highest score for the telemetry tracker card
  ...
  <div style={inlineStyle} className={`min-h-screen font-sans selection:bg-amber-500 selection:text-black relative ${themeBgClass}`}>
    ...
    {xpPopup && (
      <div className={`fixed top-24 right-8 font-black tracking-widest text-sm px-4 py-2.5 rounded-xl shadow-xl z-50 border ${primaryBtnClass}`}>
        ✨ {xpPopup.text} ({xpPopup.points > 0 ? `+${xpPopup.points}` : xpPopup.points} XP)
      </div>
    )}
    ...
    <main className="max-w-[1600px] mx-auto p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10">
      ...
      <section className="lg:col-span-4 space-y-6">
        ...
        {/* CLUTCH RATIO METRIC WRAPPER (REPLACED RAW HIGHLIGHT MODULE WITH ACTIVE PERFORMANCE TRANSITIONS) */}
        <div className={`rounded-2xl p-6 hover:-translate-y-1 hover:scale-[1.01] ${themeCardClass} ${themeBorderClass}`}>
          ...
          {/* MAIN MATRIX PROGRESSION BAR RAIL */}
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-4 mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    <button 
                      onClick={() => forceCompleteTask(task._id || task.id, task.objective)}
